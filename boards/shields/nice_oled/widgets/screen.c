...
#include <zmk/wpm.h>
    ...
    IS_ENABLED(                                                                \
        CONFIG_NICE_OLED_WIDGET_CENTRAL_SHOW_BATTERY_PERIPHERAL_ONLY) ||       \
    IS_ENABLED(                                                                \
        CONFIG_NICE_OLED_WIDGET_CENTRAL_SHOW_BATTERY_PERIPHERAL_AND_CENTRAL)
...
#if !IS_ENABLED(                                                               \
    CONFIG_NICE_OLED_WIDGET_CENTRAL_SHOW_BATTERY_PERIPHERAL_ALL) &&            \
    !IS_ENABLED(                                                               \
        CONFIG_NICE_OLED_WIDGET_CENTRAL_SHOW_BATTERY_PERIPHERAL_ONLY) &&       \
    !IS_ENABLED(                                                               \
        CONFIG_NICE_OLED_WIDGET_CENTRAL_SHOW_BATTERY_PERIPHERAL_AND_CENTRAL)
 ...
 * y lo muestra como una cadena de texto simple en las coordenadas
 * especificadas.
 */
static void draw_battery_text(lv_obj_t *canvas,
  ...
  init_label_dsc(&label_dsc, LVGL_FOREGROUND, &pixel_operator_mono_16,
                 LV_TEXT_ALIGN_LEFT);
#else
  init_label_dsc(&label_dsc, LVGL_FOREGROUND, &lv_font_unscii_8,
                 LV_TEXT_ALIGN_LEFT);
#endif

  //  Lógica Parcelada
#if IS_ENABLED(CONFIG_NICE_OLED_WIDGET_CENTRAL_SHOW_BATTERY_PERIPHERAL_ALL)
  // MODO 1: Muestra TODAS las baterías (Central + Periféricos) en una sola
  // linea
  char *p = text;
  char *end = text + sizeof(text);
  for (int i = 0; i < CONFIG_NICE_OLED_SPLIT_TOTAL_DEVICES; i++) {
    // Añade el nivel de la batería y un espacio, controlando el tamaño del
    // buffer
  ...
  lv_canvas_draw_text(canvas, 0, 19, lv_obj_get_width(canvas), &label_dsc,
                      text);
}
/*
static void draw_battery_text(lv_obj_t *canvas, const struct status_state
*state) { lv_draw_label_dsc_t label_dsc;

    // Inicialización de la fuente y el estilo del texto
#if IS_ENABLED(CONFIG_NICE_EPAPER_ON)
    init_label_dsc(&label_dsc, LVGL_FOREGROUND, &pixel_operator_mono_16,
LV_TEXT_ALIGN_LEFT); #else init_label_dsc(&label_dsc, LVGL_FOREGROUND,
&lv_font_unscii_8, LV_TEXT_ALIGN_LEFT); #endif
    ...
    snprintf(central_text, sizeof(central_text), "%d",
state->batteries[0].level); lv_canvas_draw_text(canvas, 0, 1, 25, &label_dsc,
central_text);
...
IS_ENABLED(CONFIG_NICE_OLED_WIDGET_CENTRAL_SHOW_BATTERY_PERIPHERAL_AND_CENTRAL)
    ...
    if (CONFIG_NICE_OLED_SPLIT_TOTAL_DEVICES >= 2) {
        snprintf(text, sizeof(text), "%d %d", state->batteries[0].level,
state->batteries[1].level); } else { snprintf(text, sizeof(text), "%d",
state->batteries[0].level);
    }
#endif
...
#if IS_ENABLED(                                                                \
    CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_FIXED_SYMBOL_WINDOWS)
// Windows: Control, Shift, Alt, Win
static const lv_img_dsc_t *mod_imgs_normal[4] = {&control_0, &shift_0, &alt_0,
                                                 &win_0};
static const lv_img_dsc_t *mod_imgs_active[4] = {
    &control_white_0, &shift_white_0, &alt_white_0, &win_white_0};
#else
// macOS (default): Control, Shift, Option, Command
static const lv_img_dsc_t *mod_imgs_normal[4] = {&control_0, &shift_0, &opt_0,
                                                 &cmd_0};
static const lv_img_dsc_t *mod_imgs_active[4] = {
    &control_white_0, &shift_white_0, &opt_white_0, &cmd_white_0};
#endif
#endif // CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_FIXED_SYMBOL

// Función de dibujo para los modificadores
static void draw_mods_status(lv_obj_t *canvas,
                             const struct status_state *state) {
#if IS_ENABLED(CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_FIXED_SYMBOL)
  ...
  // --- VERTICAL (Apilado) ---
#if IS_ENABLED(                                                                \
    CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_FIXED_VER_ALIGN_RIGHT)
#if IS_ENABLED(CONFIG_NICE_EPAPER_ON)
  const int base_x =
      68 - img_size - 2; // buena posicion para X vertical en right
#else
  const int base_x = 128 - img_size - 2;
#endif
#elif IS_ENABLED(                                                              \
    CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_FIXED_VER_ALIGN_LEFT)
  ...
  for (int i = 0; i < 4; i++) {
    bool selected =
        (state->mod_state >> i) & 1 || (state->mod_state >> (i + 4)) & 1;
    int current_x = base_x;
    int current_y = base_y + i * (img_size + spacing);
    const lv_img_dsc_t *img =
        selected ? mod_imgs_active[i] : mod_imgs_normal[i];
  ...
  init_label_dsc(&mod_dsc, LVGL_FOREGROUND, &lv_font_unscii_8,
                 LV_TEXT_ALIGN_CENTER);
  lv_draw_label_dsc_t mod_dsc_black;
  init_label_dsc(&mod_dsc_black, LVGL_BACKGROUND, &lv_font_unscii_8,
                 LV_TEXT_ALIGN_CENTER);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    lv_canvas_draw_rect(canvas, current_x, current_y, box_width, box_height,
                        &rect_black_dsc);
    if (selected && inner_box_width > 0 && inner_box_height > 0) {
      lv_canvas_draw_rect(canvas, current_x + inner_box_offset,
                          current_y + inner_box_offset, inner_box_width,
                          inner_box_height, &rect_white_dsc);
    }
  ...
  static const int offsets_box[4][2] = {{0, 0},
                                        {box_width + 2, 0},
                                        {0, box_height + 2},
  ...
  static const int offsets_default[4][2] = {{0, 0},
                                            {box_width + 2, 0},
                                            {0, box_height + 2},
                                            {box_width + 2, box_height + 2}};
...
static void
set_mods_status(struct zmk_widget_screen *widget,
...
// Se suscribe a los cambios de estado de las teclas (que pueden afectar a los
// modificadores)
...
static void draw_hid_status(lv_obj_t *canvas,
                            const struct status_state *state) {

#if IS_ENABLED(                                                                \
    CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_FIXED_SYMBOL_VERTICAL) ||     \
    IS_ENABLED(                                                                \
        CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_FIXED_ONE_LINE_VERTICAL)
...
#define DRAW_HID_STATUS_TEXT_ALIGN LV_TEXT_ALIGN_LEFT
  ...
  // lv_font_unscii_8
  // #define DRAW_HID_STATUS_FONTS \ (IS_ENABLED(CONFIG_NICE_EPAPER_ON) ?
  // &lv_font_montserrat_14 : &pixel_operator_mono_12)
#if IS_ENABLED(CONFIG_NICE_EPAPER_ON)
...
#define DRAW_HID_STATUS_FONTS &pixel_operator_mono_12
  ...
  // LV_TEXT_ALIGN_CENTER);
  init_label_dsc(&label_time, LVGL_FOREGROUND, DRAW_HID_STATUS_FONTS,
  ...
  if (state->is_connected) {
    ...
    lv_canvas_draw_text(canvas,
                        CONFIG_NICE_OLED_WIDGET_RAW_HID_WEATHER_CUSTOM_X,
    ...
    lv_canvas_draw_text(canvas,
                        CONFIG_NICE_OLED_WIDGET_RAW_HID_MEDIA_PLAYER_CUSTOM_X,
    ...
    lv_canvas_draw_text(canvas, hid_area_x, current_y, hid_area_width,
                        &label_time, "HID");
    current_y += text_size.y + line_gap;

    // Dibujar "not"
    lv_txt_get_size(&text_size, "not", label_layout.font,
                    label_layout.letter_space, label_layout.line_space,
    ...
    // Dibujar "found"
    lv_txt_get_size(&text_size, "found", label_volume.font,
                    label_volume.letter_space, label_volume.line_space,
                    LV_COORD_MAX, LV_TEXT_FLAG_NONE);
    lv_canvas_draw_text(canvas, hid_area_x, current_y, hid_area_width,
                        &label_volume, "found");
  }
...
static struct is_connected_notification
get_is_hid_connected(const zmk_event_t *eh) {
  // Esta función asume que el evento is_connected_notification existe y se
  // puede extraer así. Verifica que as_is_connected_notification sea la forma
  // correcta de obtener este evento.
  struct is_connected_notification *notification =
      as_is_connected_notification(eh);
  if (notification) {
    return *notification;
  }
  // Devuelve un estado desconectado por defecto si el evento no es del tipo
  // esperado o si el puntero es NULL (puede pasar durante la inicialización).
  return (struct is_connected_notification){.value = false};
}

static void
hid_is_connected_update_cb(struct is_connected_notification is_connected) {
...
ZMK_DISPLAY_WIDGET_LISTENER(widget_is_connected,
...
#ifdef CONFIG_NICE_OLED_WIDGET_RAW_HID_LAYOUT // Reutiliza la Kconfig de
                                              // status.c
...
static struct weather_notification
weather_status_get_state(const zmk_event_t *eh) {
...
static struct spotify_notification
spotify_status_get_state(const zmk_event_t *eh) {
...
static void set_battery_status(struct zmk_widget_screen *widget,
  ...
  LOG_DBG("Source: %d, level: %d, usb: %d", state.source, state.level,
          state.usb_present);
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
  ...
  lv_obj_align(zmk_widget_luna_obj(&luna_widget), LV_ALIGN_TOP_LEFT,
  ...
  lv_obj_align(zmk_widget_wpm_bongo_cat_obj(&wpm_bongo_cat_widget),
               LV_ALIGN_TOP_LEFT, CONFIG_NICE_OLED_WIDGET_BONGO_CAT_CUSTOM_X,
  ...
  lv_obj_align(
      zmk_widget_responsive_bongo_cat_obj(&responsive_bongo_cat_widget),
      LV_ALIGN_TOP_LEFT, CONFIG_NICE_OLED_WIDGET_RESPONSIVE_BONGO_CAT_CUSTOM_X,
...
#if IS_ENABLED(CONFIG_NICE_OLED_WIDGET_MODIFIERS_INDICATORS_LUNA)
  zmk_widget_modifiers_init(&modifiers_widget,
                            canvas); // Inicializar el widget de modifiers
#endif
  ...
  // Nota: Los valores iniciales (hora, vol, etc.) se obtendrán cuando lleguen
  ...
  lv_obj_align(zmk_widget_sleep_status_obj(&sleep_status_widget),
               LV_ALIGN_TOP_LEFT, CONFIG_NICE_OLED_WIDGET_SLEEP_STATUS_CUSTOM_X,
